package com.example.nexnote.data.db
...
interface TagDao {
    ...
    /** All note IDs (including trashed) that reference [tagName]. */
    @Query("SELECT DISTINCT noteId FROM note_tag_cross_ref WHERE tagName = :tagName")
    suspend fun getNoteIdsForTag(tagName: String): List<Long>

...
import com.example.nexnote.data.db.TagDao
import com.example.nexnote.data.db.TagWithCount
import com.example.nexnote.data.db.entity.NoteTagCrossRef
import com.example.nexnote.data.db.entity.TagEntity
import com.example.nexnote.domain.model.Tag
import com.example.nexnote.util.TagParser
 ...
 *
 * [noteDao] is injected alongside [tagDao] because tag deletion must update note
 * content directly (stripping '#' prefixes) without going through [NoteRepository],
 * which would trigger a full note-save cycle and re-index tags unnecessarily.
 *
...
class TagRepository(
...
) : com.example.nexnote.domain.repository.TagRepository {
    ...
    override suspend fun indexNoteTags(noteId: Long, content: String) {
        require(noteId > 0L) { "indexNoteTags called with unsaved noteId=0" }
    ...
    override suspend fun deleteTag(tagName: String) {
        ...
        val noteIds = tagDao.getNoteIdsForTag(tagName)
        val now = System.currentTimeMillis()

        for (noteId in noteIds) {
            val entity = noteDao.getNoteById(noteId) ?: continue
            val newContent = tagRegex.replace(entity.content) { match ->
                // Replace '#tagName' with 'tagName' while preserving the leading whitespace.
                match.value.replace("#$tagName", tagName)
            }
            if (newContent != entity.content) {
                noteDao.updateNoteContent(noteId, newContent, now)
            }
        }

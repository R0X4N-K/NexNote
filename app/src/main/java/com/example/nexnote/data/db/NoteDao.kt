...
interface NoteDao {
    ...
    /**
     * Updates the content of a specific note and refreshes [lastModifiedDate].
     * Used by the tag deletion flow to strip '#' prefixes from note content
     * without going through the full note-save pipeline.
     */
    @Query("UPDATE notes SET content = :content, lastModifiedDate = :lastModifiedDate WHERE id = :id")
    suspend fun updateNoteContent(id: Long, content: String, lastModifiedDate: Long)
}
